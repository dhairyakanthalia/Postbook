...
export class ViewpostComponent implements OnInit {
  ...
  postval!:number;
  constructor(private userinfo : UserinfoService,private dialog: MatDialog) {
  ...
  ngOnInit(): void {
    ...
    this.userinfo.getpost().subscribe((data)=>{ this.posts = data.reverse()})
  ...
  oncommentclick(id:number | undefined){
    for (let index = this.posts.length-1; index >=0; index--) {
      if(this.posts[index].id === id){
   this.dialog.open(CommentComponent,{data:[this.posts[index],this.user]});
      }
    }
  }
    ...
    if(this.posts[index].id === id){
      ...
      if(this.liked.length>0){
        ...
        for(let i = 0;i<this.liked.length;i++){    
          if(this.liked[i].userid === this.user.id && this.liked[i].postid === id){
            this.flag=true;
            this.dislikeclick = true;
        ...
        if(this.posts[index].id === id){
          ...
          if(this.disliked.length>0){
            ...
            for(let i = 0;i<this.disliked.length;i++){
              if(this.disliked[i].userid === this.user.id && this.disliked[i].postid === id){
                this.flag=true;
                this.likeclick=true;
